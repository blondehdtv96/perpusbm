...
export default function UsersPage() {
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-3"><h2 className="text-xl font-black text-navy-950">Daftar anggota</h2><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{pagination.total} data</span></div><p className="mt-1 text-sm text-slate-500">Cari dan kelola akun anggota perpustakaan.</p></div><div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row"><button type="button" onClick={() => togglePage(!allPageSelected)} disabled={users.length === 0 || loading || printingCards} className="min-h-11 rounded-xl border border-blue-200 px-4 text-sm font-bold text-blue-700 hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-40">{allPageSelected ? 'Batalkan halaman' : 'Pilih halaman ini'}</button><button type="button" onClick={printCards} disabled={selected.length === 0 || loading || printingCards} className="min-h-11 rounded-xl bg-blue-700 px-5 text-sm font-bold text-white shadow-sm hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-40">{printingCards ? 'Membuat PDF…' : `Cetak kartu (${selected.length})`}</button></div></div></div>
