...
export default function UsersPage() {
  ...
  return <>
    <div><p className="text-sm font-bold text-emerald-700">ADMINISTRASI</p><h1 className="mt-1 text-3xl font-black">Anggota</h1><p className="mt-2 text-slate-500">Kelola akun, status, dan impor data anggota dengan lebih mudah.</p></div>
    ...
    {canCreate && <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(20rem,1fr)]">
      <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div><h2 className="text-lg font-black">Tambah anggota</h2><p className="mt-1 text-sm text-slate-500">Tambahkan satu anggota secara manual.</p></div><form onSubmit={create} className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <button disabled={busy} className="min-h-11 self-end rounded-xl bg-emerald-700 px-4 font-bold text-white disabled:opacity-60">{busy ? 'Memproses…' : 'Simpan anggota'}</button>
      </form></section>

      <section className="rounded-3xl border border-slate-200 bg-white p-5 sm:p-6"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-black">Import anggota</h2><p className="mt-1 text-sm text-slate-500">Gunakan template agar data langsung sesuai format.</p></div><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">XLSX</span></div>
        <ol className="mt-4 grid grid-cols-3 gap-2 text-center text-[11px] font-bold text-slate-500"><li className="rounded-xl bg-slate-50 p-2"><span className="block text-base text-emerald-700">1</span>Unduh</li><li className="rounded-xl bg-slate-50 p-2"><span className="block text-base text-emerald-700">2</span>Isi data</li><li className="rounded-xl bg-slate-50 p-2"><span className="block text-base text-emerald-700">3</span>Unggah</li></ol>
        <button type="button" onClick={downloadTemplate} disabled={downloadingTemplate || busy} className="mt-4 min-h-11 w-full rounded-xl border border-emerald-700 px-4 text-sm font-bold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50">{downloadingTemplate ? 'Mengunduh…' : '↓ Unduh template XLSX'}</button>
        <div role="button" tabIndex="0" onClick={() => fileInputRef.current?.click()} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') fileInputRef.current?.click() }} onDragEnter={(event) => { event.preventDefault(); setDragging(true) }} onDragOver={(event) => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); selectImportFile(event.dataTransfer.files?.[0]) }} className={`mt-4 cursor-pointer rounded-2xl border-2 border-dashed p-5 text-center transition ${dragging ? 'border-emerald-500 bg-emerald-50' : 'border-slate-300 bg-slate-50 hover:border-emerald-400'}`}><div className="text-2xl" aria-hidden="true">⇧</div><p className="mt-2 text-sm font-bold">Tarik file ke sini atau klik untuk memilih</p><p className="mt-1 text-xs text-slate-500">CSV, XLSX, atau XLS • Maksimal 5 MB</p><input ref={fileInputRef} type="file" accept=".csv,.txt,.xlsx,.xls" onChange={(event) => selectImportFile(event.target.files?.[0])} className="sr-only" /></div>
        {importFile && <div className="mt-3 flex items-center justify-between gap-3 rounded-xl bg-emerald-50 p-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-emerald-900">{importFile.name}</p><p className="text-xs text-emerald-700">{(importFile.size / 1024).toFixed(1)} KB • Siap diunggah</p></div><button type="button" onClick={() => { setImportFile(null); if (fileInputRef.current) fileInputRef.current.value = '' }} className="shrink-0 text-xs font-bold text-red-600">Hapus</button></div>}
        <button type="button" onClick={importUsers} disabled={!importFile || busy} className="mt-3 min-h-11 w-full rounded-xl bg-slate-900 px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{busy && importFile ? 'Mengimpor data…' : 'Mulai import'}</button>
      </section>
    </div>}
...
function Field({ label, value, onChange, type = 'text', required = false, minLength }) {
  return <label className="text-sm font-bold">{label}<input type={type} value={value} required={required} minLength={minLength} onChange={(event) => onChange(event.target.value)} className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-normal outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100" /></label>
}

function Select({ label, value, onChange, options }) {
  return <label className="text-sm font-bold">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 font-normal outline-none focus:border-emerald-600">{options.map(([itemValue, itemLabel]) => <option key={itemValue} value={itemValue}>{itemLabel}</option>)}</select></label>
}
